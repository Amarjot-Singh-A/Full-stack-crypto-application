...
const getFavourite = async (req, res) => {
  try {
    ...
    if (Array.isArray(result) && result.length > 0) {
      logger.info(
        `Favourite coins fetched successfully for user ${req.session.userId}`,
      );
    ...
    logger.error(
      'Error fetching favourite coins - favouriteCoinsController',
      err.message || err,
    );
    res.status(500).send({
      result: [],
...
const createFavourite = async (req, res) => {
  try {
    ...
    if (Array.isArray(result) && result.length > 0) {
      logger.info(
        `Favourite coin added successfully for user ${req.session.userId}`,
      );
      ...
      logger.warn(
        `Failed to add favourite coin for user ${req.session.userId}`,
      );
      res.status(400).send({
        result,
        error:
          'Failed to add favourite coin. It may already exist or input is invalid.',
      });
    }
  } catch (err) {
    logger.error(
      'Error inserting favourite coin - favouriteCoinsController',
      err.message || err,
    );
    res.status(500).send({
      result: [],
