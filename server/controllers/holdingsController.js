...
const getHoldings = async (req, res) => {
  try {
    ...
    if (Array.isArray(result) && result.length > 0) {
      logger.info(`Holdings fetched successfully for user ${req.session.userId}`);
    ...
    logger.error("Error getting holdings - holdingsController", error.message || error);
    res.status(500).send({
      result: [],
      error: `Error getting holding - controller level : ${error.message}`,
    });
...
const createHoldings = async (req, res) => {
  try {
    ...
    if (Array.isArray(result) && result.length > 0) {
      logger.info(`Holding created successfully for user ${req.session.userId}`);
      return res.status(201).send({
        result,
        error,
      });
    } else {
      logger.error("Error inserting holding - controller level", error?.message || error);
      res.status(400).send({
        result,
    ...
    logger.error("Error inserting holding - holdingsController", err.message || err);
    res.status(500).send({
      result: [],
...
const removeHoldings = async (req, res) => {
  try {
    ...
    if (Array.isArray(result) && result.length > 0) {
      ...
      logger.error("Error deleting holdings - controller level", error?.message || error);
      res.status(400).send({
        result,
    ...
    logger.error("Error deleting holdings - holdingsController", err.message || err); 
    res.status(500).send({
      result: [],
